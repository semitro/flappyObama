...
Food::Food(FoodType type){
	...
	else if(type == Nirvana)
		texture.loadFromFile("Images/Nirvana.png");
	else if(type == Rammstein)
	...
	// Чтоб размер был не совсем одинаковый
	...
	if(type == Matryoshka)
		...
		_rotation_factor = (rand()%60-30)/10;sprite->scale(0.64+randomScale,0.64+randomScale);}
	else if(type == Vodka)
		sprite->scale(0.3+randomScale,0.3+randomScale);
	else if(type == Gambyrger)
